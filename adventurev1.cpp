...
int main()
{
    ...
    while ( currentRoom->isExit() != true)
    {
        ...
        for(Door doorNumber = *currentDoor; currentRoom->isValidDoor(currentRoom->getNextDoor(*currentDoor)); currentDoor = &(currentRoom->getNextDoor(*currentDoor)))
        {
    ...
    std::cout << p1; //Runs if exit reached successfully
    return 0;
}
