...
int main()
{
    ...
    for (int loop{1}; loop <= listsToInput; loop++) //start at 1 so numberElements[0] == 0 && usedElements[0] == 0
    {
        ...
        //cout << "usedElements: " << usedElements[loop] << endl;
    ...
    usedElements.resize( usedElements.size() + 1);
    activeElements.resize( usedElements.size() + 10 );
    usedElements[listsToInput + 1] = usedElements[listsToInput] + 1;
    activeElements[usedElements[listsToInput]] = 0;
    ...
    for (int loop{0}; loop < listsToInput; loop++)
    {
        ...
        //cout << "position (new loop): " << position << endl;
        ...
        while (1==1)    //intentional infinite loop, breaks when game ends
        {
            ...
            if (revisitCheck[position] > 1)        //infinite loop
            {
                cout << "invalid\n";
                break;
            }
            if ((position + *currentNumber) >= usedElements[loop+1] ) 
                position = usedElements[loop + 1]; 
            else 
                position += *currentNumber;
            //cout << "nextNumber: " << nextNumber << '\n';
            //position += *currentNumber;
            //cout << "position: " << position << '\n';
            *currentNumber = activeElements[position];
